...
std::string square_name(Square s) {
    ...
    auto file = square_file(s);
    auto rank = square_rank(s);
...
static void initialize_distances() {
    for (auto a = Square(0); a < SQ_COUNT; ++a) {
        for (auto b = Square(0); b < SQ_COUNT; ++b) {
            auto file_dist = std::abs(square_file(a) - square_file(b));
            auto rank_dist = std::abs(square_rank(a) - square_rank(b));
    ...
    for (auto s = Square(0); s < SQ_COUNT; ++s) {
        auto distance = g_manhattan[s][SQ_E4];
...
static void initialize_between() {
    ...
    for (auto a = Square(0); a < SQ_COUNT - 1; ++a) {
        g_between[a][a] = 0;
        for (auto b = Square(a + 1); b < SQ_COUNT; ++b) {
            auto file_a = square_file(a);
            auto rank_a = square_rank(a);

            auto file_b = square_file(b);
            auto rank_b = square_rank(b);

            auto x_delta = file_b - file_a;
            auto y_delta = rank_b - rank_a;
            ...
            if (y_delta == 0) {
                // Horizontal
                for (auto s = Square(a + DIR_EAST); s < b; s += DIR_EAST) {
                    bb = set_bit(bb, s);
                }
            }
            else if (x_delta == 0) {
                // Vertical
                for (auto s = Square(a + DIR_NORTH); s < b; s += DIR_NORTH) {
                    bb = set_bit(bb, s);
                }
            }
                ...
                // Diagonal
                if (x_delta < 0) {
                    for (auto s = Square(a + DIR_NORTHWEST); s < b; s += DIR_NORTHWEST) {
                        bb = set_bit(bb, s);
                    }
                }
                else { // deltaX > 0
                    for (auto s = Square(a + DIR_NORTHEAST); s < b; s += DIR_NORTHEAST) {
                        bb = set_bit(bb, s);
                    }
                }
            }
...
Move Move::parse_uci(const Board& board, std::string_view move_str) {
    ...
    auto src = parse_square(move_str);
    if (src == SQ_NULL) {
        return MOVE_NULL;
    }
    auto dst = parse_square(move_str.substr(2));
    if (dst == SQ_NULL) {
        return MOVE_NULL;
    }

    auto prom_piece_type = PieceType(PT_NULL);
    if (move_str.size() > 4) {
        // Try to parse a promotion piece.
        auto p = Piece::from_char(move_str[4]);
        if (p == PIECE_NULL) {
            return MOVE_NULL;
        }
...
Move::Move(const Board& board, Square src, Square dst, PieceType prom_piece_type) {
    ...
    if (src_piece.type() == PT_PAWN) {
        ...
        if (square_file(src) == square_file(dst)) {
            ...
            auto delta = std::abs(square_rank(dst) - square_rank(src));
            if (delta == 2) {
        ...
        auto king_color = src_piece.color();
        auto file_delta = square_file(dst) - square_file(src);
        ...
        if (file_delta > 1) {
            ...
            // as the moving king.
        auto side = dst == board.castle_rook_square(king_color, SIDE_KING)
                             ? SIDE_KING
                             : SIDE_QUEEN;
