...
private:
    ...
    struct Data {
        ...
        std::array<std::array<ButterflyArray<int>, 2>, 2> main_history {};
...
inline int MoveHistory::quiet_history(Move move, Move last_move, bool threatened_from, bool threatened_to) const {
    return int(
               i64(MV_HIST_REGULAR_QHIST_WEIGHT * m_data->main_history[threatened_from][threatened_to].get(move))
...
inline void MoveHistory::update_quiet_history(Move move,
                                              ...
                                              bool threatened_to) {
    update_history_by_depth(m_data->main_history[threatened_from][threatened_to].get(move), depth, good);
