            ...
            if constexpr (!METAMORPHIC) {
                if (n_expected_moves > 0) {
                    ...
                    for (auto& h: history) {
                        ...
                        mv_hist.update_quiet_history(Move::new_normal(src, dest, WHITE_QUEEN), MOVE_NULL, depth, true, TODO, TODO);
                    }
                }
            }
            else {
                if (n_expected_moves > 0) {
                    ...
                    for (int i = 0; i < 100; ++i) {
                        // Pick a move.
                        Move move = validation_moves[random(size_t(0), n_expected_moves)];
                        mv_hist.update_quiet_history(move, MOVE_NULL, random(1, 15), true, TODO, TODO);
                    }
                }
                else {
                    hash_move = MOVE_NULL;
